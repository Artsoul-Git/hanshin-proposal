...
/visualize 羅針盤 [テーマ]    → モードE（目標達成コンパス）実行
/rashinban [テーマ]            → モードEの省略形
```

---

## モードE：目標達成コンパス（羅針盤モード）

**起動：** `/visualize 羅針盤 [テーマ]` または `/rashinban [テーマ]`  
**用途：** 目標・課題を起点にヒアリング→AI分析→マンダラ→マトリクス→フロー→ガントを一気通貫で生成。Kaiがコンサルタント兼伴走者として全面分析コメントを埋め込む。  
**出力：** インタラクティブHTML（全チャートをタブで切り替え可、ドリルダウン可、チェックリスト付き）

### テンプレートファイル

`08_アプリ開発事業部/outputs/rashinban/rashinban-template.html`

**このファイルのDATAオブジェクト内のすべての`REPLACE_`マーカーを実際の内容に置き換えてHTMLを生成する。**

---

### STEP 1：ヒアリング（5問・会話形式）

テーマを受け取ったら、以下の質問を**1問ずつ順番に**会話で聞く（まとめて聞かない）：

```
Q1: 「[テーマ]」を一文でゴール状態として言い直すとどうなりますか？
    （例：「売上2倍」→「2026年12月末時点で月商500万円を安定達成する」）

Q2: 現在の状況を教えてください。
    ・今どこにいて（現状）
    ・どこに行きたいか（ゴール）のギャップは何ですか？

Q3: 達成したとき、何が変わっている・測定できるものは何ですか？
    （成功指標・KPIを具体的に）

Q4: 目標期間と主な制約を教えてください。
    （時間・予算・人・情報など）

Q5: 過去に試みたこと、すでにうまくいっていることはありますか？
    （強みと失敗パターンの把握）
```

回答を受け取ったら「では分析します」と一言告げて生成へ進む。確認は不要。

---

### STEP 2：Kaiコンサルタント分析（生成前に行う内部推論）

ヒアリング回答をもとに以下を整理してから生成する（出力に見せる必要はない）：

```
・コアイシュー：本当の問題は何か（表面の問題ではなく根本）
・勝ち筋：この目標達成で最も効くレバーはどこか
・最優先アクション：今すぐやるべき1つのこと
・強み3つ：活かせるリソース・実績・状況
・リスク3つ：失敗パターン・見落としがちな落とし穴
・テーマ別洞察：8テーマそれぞれへの2〜3文コメント
・厳しいひと言：コンサルタントとして忖度なしに言うべき最重要の指摘
```

---

### STEP 3：マンダラ8テーマの設計原則

目標達成に必要な8つの柱を**MECE**に設計する：

```
・互いに重複しない（Mutually Exclusive）
・合わせて目標達成を網羅する（Collectively Exhaustive）
・抽象（戦略）と具体（実行）のバランスをとる
・「やること」だけでなく「強化すること」「排除すること」も含める
・上村の事業文脈を反映する（AI導入支援・コンサル業）
```

各テーマから8つのアクション項目を展開（計64項目）。  
priorityは `1=最優先` `2=重要` `3=中長期` で設定。

---

### STEP 4：マトリクス設計（8テーマ分）

各テーマの8アクションを4象限に分類する：

| 象限 | 位置 | 意味 | pos値 |
|------|------|------|-------|
| 🔴 今すぐ着手 | 右上（高重要・高緊急） | 最優先で着手 | `tr` |
| 🔵 計画的推進 | 左上（高重要・低緊急） | 期間を決めて計画実行 | `tl` |
| 🟡 委任・効率化 | 右下（低重要・高緊急） | 外注・自動化を検討 | `br` |
| ⚪ 保留・見直し | 左下（低重要・低緊急） | 今は手を出さない | `bl` |

各象限に2〜3項目配置。`kaiInsight` には2〜3文のKaiアドバイスを書く。

---

### STEP 5：フロー図設計（「今すぐ着手」項目ごと）

各テーマの「🔴 今すぐ着手」象限の主要項目1〜2つにフロー図を作成する：

```
・steps: 5〜8ステップ
・type: action（通常） / decision（判断・分岐） / milestone（節目） / warning（注意）
・duration: 所要時間の目安（「3日」「2週間」など）
・output: このステップで生まれる成果物
・sourceItem: matrixのitems.textと完全一致させる（クリック連携のため）
・kaiComment: このフローへのコメント（2〜3文。何が肝か・何に注意するか）
```
...
全体実行計画を totalWeeks: 24（約6ヶ月）で設計：

```
・3〜5フェーズに分ける（準備→立上げ→実行→定着→拡大など）
・タスク名は簡潔（10〜15文字以内）
・クリティカルパス（isCritical: true）は2〜4タスクに限定
・マイルストーン（milestone: true）を各フェーズ末尾に1つ設置
・startWeek / endWeek は1〜24の整数
```
...
1. テンプレートファイルを Read して全文を取得する
2. すべての `REPLACE_` マーカーを実際の内容に置き換える
3. DATAオブジェクトのコメント（`/* ... */`）を実際のデータ配列に置き換える
4. 以下のパスに保存する：

```
保存先：08_アプリ開発事業部/outputs/rashinban/
ファイル名：{テーマ（日本語可）}_羅針盤_{YYYYMMDD}.html
例：売上2倍_羅針盤_20260516.html
```

5. 保存後すぐにブラウザで開く（`start` コマンド or `open` コマンド）

---

### STEP 8：生成後の案内

HTMLを開いた後、以下を1〜2文で案内する：

```
「[テーマ]の羅針盤を生成しました。
マンダラのテーマセルをクリックするとマトリクス、
項目をクリックするとフロー図に移動できます。
フローのチェックボックスで実行進捗が追跡されます（ブラウザ保存）。
修正・追加が必要な点があれば教えてください。」
```
...
- [ ] ヒアリング5問が全て回答済みか
- [ ] kai.summary は200字程度か（多すぎず短すぎず）
- [ ] kai.consultantVerdict は忖度なしの指摘になっているか（「〜が課題です」ではなく「〜しないと失敗する」レベル）
- [ ] 8テーマがMECEか（重複・抜け漏れなし）
- [ ] 各テーマに8アクション（計64項目）あるか
- [ ] priority 1 が各テーマに少なくとも1〜2個あるか
- [ ] matrixのsourceItemとflowsのsourceItemが完全一致しているか
- [ ] ガントのstartWeek/endWeekが1〜24の範囲内か
- [ ] すべての REPLACE_ マーカーが置き換えられているか
